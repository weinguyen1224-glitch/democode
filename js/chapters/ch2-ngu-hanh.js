...
function ch2NguHanhInit() {
  ...
  const NGU_HANH = {
    moc: {
      name: "🌿 MỘC",
      color: "#4A7C59",
      badge: "Lá dong, lá chuối - Sinh sôi, sức sống",
      meaning:
        "Bao bọc bên ngoài chiếc bánh là lớp lá dong, lá chuối xanh mướt - sắc màu của cỏ cây, của sự sinh sôi và sức sống bền bỉ. Trong quan niệm ngũ hành, Mộc tượng trưng cho sự phát triển, cho mái ấm gia đình luôn đơm hoa kết trái và tràn đầy sinh khí.",
    },
    kim: {
      name: "⬡ KIM",
      color: "#8A8070",
      badge: "Dừa nạo, đường phèn - Tinh khiết, thủy chung",
      meaning:
        "Vị ngọt thanh hòa quyện cùng những sợi dừa trắng trong nhân bánh gợi liên tưởng đến hành Kim. Đây là biểu tượng của sự tinh khiết, thủy chung và bền vững - những giá trị cốt lõi trong tình nghĩa phu thê.",
    },
    tho: {
      name: "◈ THỔ",
      color: "#D4A843",
      badge: "Nhân đậu xanh, quả dành dành - Ổn định, ấm áp",
      meaning:
        "Màu vàng óng của lớp vỏ bánh được nhuộm từ quả dành dành tự nhiên hòa quyện với sắc vàng của nhân đậu xanh bên trong. Hành Thổ tượng trưng cho sự ổn định, ấm áp và là nền tảng vững chắc nâng đỡ hạnh phúc gia đình qua năm tháng.",
    },
    hoa: {
      name: "🔥 HỎA",
      color: "#972023",
      badge: "Sợi lạt đỏ - Niềm vui, gắn kết",
      meaning:
        "Nổi bật trên nền xanh của lớp lá là sợi lạt đỏ thắm buộc chéo quanh cặp bánh. Sắc đỏ của hành Hỏa tượng trưng cho niềm vui ngày cưới, cho ngọn lửa yêu thương và sự gắn kết nồng ấm của đôi lứa.",
    },
    thuy: {
      name: "💧 THỦY",
      color: "#6B9BC3",
      badge: "Nước - Hài hòa, linh hoạt, bền bỉ",
      meaning:
        "Để tạo nên lớp vỏ trong veo, mềm mại đặc trưng của bánh phu thê không thể thiếu nước - yếu tố đại diện cho hành Thủy. Thủy là biểu tượng của sự hài hòa, linh hoạt và dòng chảy cảm xúc bền bỉ, kết nối hai con người trong cuộc sống hôn nhân.",
    },
  };
  ...
  const HANH_BURST = [
    {
      ...
      angle: -90,
      dist: 320,
      colors: ["#40c030", "#80e050", "#b0f070"],
    },
    {
      key: "kim",
      angle: -18,
      dist: 340,
      colors: ["#c8e0f8", "#e0f0ff", "#ffffff"],
    },
    {
      key: "tho",
      angle: 54,
      dist: 330,
      colors: ["#f0c010", "#f8e040", "#fff080"],
    },
    {
      key: "hoa",
      angle: 126,
      dist: 340,
      colors: ["#972023", "#C41E3A", "#E84050"],
    },
    {
      key: "thuy",
      angle: 198,
      dist: 320,
      colors: ["#1890e0", "#30c0f8", "#70d8ff"],
    },
  ];
  ...
  if (typeof IMGS !== "undefined") {
    ...
    const assetMap = {
      "bpt-img-ladong": "assets/background-remover/ladong.webp",
      "bpt-img-dua": "assets/background-remover/dua.webp",
      "bpt-img-duong": "assets/background-remover/duong.webp",
      "bpt-img-dau": "assets/background-remover/dau.webp",
      "bpt-img-soi-lat": "soi-lat-do",
    };
    Object.entries(assetMap).forEach(([elId, src]) => {
      const el = document.getElementById(elId);
      if (!el) return;
      if (src === "soi-lat-do" && IMGS["soi-lat-do"])
        el.src = IMGS["soi-lat-do"];
      else el.src = src;
    });
  }
  ...
  function positionInfoPanel(cardEl) {
    ...
    if (isMobile()) {
      ...
      const leftOffset = Math.max(
        8,
        Math.min(oRect.width - panelW - 8, (oRect.width - panelW) / 2),
      );
    ...
    if (absDy > absDx) {
      ...
      if (dy < 0) {
        ...
        if (topOffset < 4) {
          ...
          if (
            ...
            leftOffset < 4 ||
            leftOffset + panelW > oRect.width - 4
          ) {
            leftOffset = Math.max(
              4,
              Math.min(oRect.width - panelW - 4, cRect.left - oRect.left),
            );
          }
        }
      } else {
      ...
      if (leftOffset === undefined) {
        leftOffset = Math.max(
          4,
          Math.min(
            oRect.width - panelW - 4,
            cRect.left - oRect.left + (cRect.width - panelW) / 2,
          ),
        );
      }
    } else {
  ...
  function burstHanh() {
    ...
    HANH_BURST.forEach((h, i) => {
      ...
      const clampedDx = Math.max(
        -oW / 2 + halfW + 4,
        Math.min(dx, oW / 2 - halfW - 4),
      );
      const clampedDy = Math.max(
        -oH / 2 + halfW + 4,
        Math.min(dy, oH / 2 - halfW - 4),
      );
      ...
      gsap.to(el, {
        ...
        onStart: () =>
          !prefersReducedMotion() &&
          addParticles(20, h.colors, canvas.width / 2, canvas.height / 2, 5),
        onComplete: () => {
      ...
      if (nameEl)
        gsap.to(nameEl, { opacity: 1, duration: 0.4, delay: delay + 0.45 });
      if (badge)
        gsap.to(badge, { opacity: 1, y: 0, duration: 0.4, delay: delay + 0.6 });
      if (desc)
        gsap.to(desc, { opacity: 1, duration: 0.35, delay: delay + 0.75 });
    });
  ...
  function collapseHanh() {
    ...
    HANH_BURST.forEach((h, i) => {
      ...
      if (nameEl)
        gsap.to(nameEl, { opacity: 0, duration: 0.2, delay: i * 0.04 });
      if (badge)
        gsap.to(badge, { opacity: 0, y: 6, duration: 0.2, delay: i * 0.04 });
    ...
    setTimeout(
      () => {
        ...
        !prefersReducedMotion() &&
          addParticles(
            40,
            ["#f8d820", "#f0a010", "#e06010", "#a0d020", "#20c060", "#60a8f0"],
            canvas.width / 2,
            canvas.height / 2,
            6,
          );
  ...
  section.addEventListener("click", (e) => {
    ...
    if (!card) {
      if (activeHanh) {
        ...
        if (infoPanel) infoPanel.classList.remove("is-visible");
        section
          .querySelectorAll(".bpt-hanh-card")
  ...
  function startUnwrap() {
    ...
    const steps = [
      {
        from: 0,
        to: 1,
        ripple: "rgba(220,80,60,0.7)",
        pColor: ["#cc4020", "#e86040", "#ff8060"],
        pN: 25,
        pSp: 3.5,
      },
      {
        from: 1,
        to: 2,
        ripple: "rgba(210,70,50,0.5)",
        pColor: ["#cc3818", "#e05030"],
        pN: 20,
        pSp: 3,
      },
      {
        from: 2,
        to: 3,
        ripple: "rgba(60,180,60,0.5)",
        pColor: ["#40b030", "#70c840", "#a8e060"],
        pN: 30,
        pSp: 4.5,
      },
      {
        from: 3,
        to: 4,
        ripple: "rgba(40,160,40,0.5)",
        pColor: ["#30a020", "#60c030", "#90d858"],
        pN: 35,
        pSp: 4,
      },
      {
        from: 4,
        to: 5,
        ripple: "rgba(240,200,40,0.6)",
        pColor: ["#d0a010", "#f0c820", "#f8e060"],
        pN: 45,
        pSp: 5,
      },
      {
        from: 5,
        to: 6,
        ripple: "rgba(240,180,20,0.7)",
        pColor: ["#f0c010", "#f8d840", "#fff090", "#e09010"],
        pN: 60,
        pSp: 6,
      },
    ];
    ...
    if (prefersReducedMotion()) {
      ...
      function rmNext() {
        ...
        if (frames[rs.to]) {
          frames[rs.to].style.opacity = "1";
          frames[rs.to].style.transform = "none";
        }
        setProgress((ri + 1) / 6);
        ri++;
    ...
    function runStep(i) {
      if (i >= steps.length) {
        gsap.to(glow, {
          background:
            "radial-gradient(circle, rgba(240,180,40,0.28) 0%, transparent 70%)",
          duration: 0.5,
          yoyo: true,
          repeat: 1,
        });
